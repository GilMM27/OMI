...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				// Main -> recuerdame.recuerdame
				uint32_t __cookie;
				int recuerdame_x;
				readfull(&__in, &recuerdame_x, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				recuerdame(recuerdame_x);
			...
			case 0x71651df7u: {
				...
				uint32_t __cookie;
				int soy_x;
				readfull(&__in, &soy_x, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "\\\\.\\pipe\\libinteractive_recuerdame_recuerdame_in", O_RDONLY);
	openstream(&__out, "\\\\.\\pipe\\libinteractive_recuerdame_recuerdame_out", O_WRONLY);
