...
static struct __stream __recuerdame_in, __recuerdame_out;

void __NOINLINE __libinteractive_init() {
	openstream(&__recuerdame_in, "\\\\.\\pipe\\libinteractive_Main_recuerdame_in", O_WRONLY);
	openstream(&__recuerdame_out, "\\\\.\\pipe\\libinteractive_Main_recuerdame_out", O_RDONLY);
}
...
static void __message_loop_recuerdame(int __current_function, int __noreturn) {
	uint32_t __msgid;
	while (readfull(&__recuerdame_out, &__msgid, sizeof(__msgid), 0)) {
...
void recuerdame(int x) {
	...
	writefull(&__recuerdame_in, &__msgid, sizeof(__msgid));
	writefull(&__recuerdame_in, &x, sizeof(int));
	writefull(&__recuerdame_in, &__cookie, sizeof(__cookie));
	writeflush(&__recuerdame_in);

	__message_loop_recuerdame(__msgid,  0 );
	uint32_t __cookie_result = 0;
	readfull(&__recuerdame_out, &__cookie_result, sizeof(__cookie_result), 1);
...
bool soy(int x) {
	...
	__message_loop_recuerdame(__msgid,  0 );
	bool __ans = false;
	readfull(&__recuerdame_out, &__ans, sizeof(__ans), 1);
	uint32_t __cookie_result = 0;
	readfull(&__recuerdame_out, &__cookie_result, sizeof(__cookie_result), 1);
