...
static void __message_loop_Main(int __current_function, int __noreturn) {
	...
	while (readfull(&__in, &__msgid, sizeof(__msgid), 0)) {
		...
		switch (__msgid) {
			case 0x13857295u: {
				// Main -> fiesta.BusquedaCool
				uint32_t __cookie;
				int BusquedaCool_n;
				readfull(&__in, &BusquedaCool_n, sizeof(int), 1);
				readfull(&__in, &__cookie, sizeof(__cookie), 1);

				int result =
				BusquedaCool(BusquedaCool_n);
...
int main(int argc, char* argv[]) {
	...
	openstream(&__in, "\\\\.\\pipe\\libinteractive_fiesta_fiesta_in", O_RDONLY);
	openstream(&__out, "\\\\.\\pipe\\libinteractive_fiesta_fiesta_out", O_WRONLY);
