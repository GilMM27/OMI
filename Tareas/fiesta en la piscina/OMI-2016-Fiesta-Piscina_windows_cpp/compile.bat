...
SET TARGET=libinteractive\fiesta\fiesta.exe
SET SOURCES=fiesta.cpp libinteractive\fiesta\fiesta_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -O2 -g -Wno-unused-result -Ilibinteractive/fiesta
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)

SET TARGET=libinteractive\fiesta\fiesta_debug.exe
SET SOURCES=fiesta.cpp libinteractive\fiesta\fiesta_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -g -Wno-unused-result -Ilibinteractive/fiesta
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)
