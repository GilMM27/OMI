...
SET TARGET=libinteractive\ColaInteractivo\ColaInteractivo.exe
SET SOURCES=ColaInteractivo.cpp libinteractive\ColaInteractivo\ColaInteractivo_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -O2 -g -Wno-unused-result -Ilibinteractive/ColaInteractivo
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)

SET TARGET=libinteractive\ColaInteractivo\ColaInteractivo_debug.exe
SET SOURCES=ColaInteractivo.cpp libinteractive\ColaInteractivo\ColaInteractivo_entry.cpp
call:recompile %TARGET% %SOURCES%
IF "%RECOMPILE%" == "1" (
<NUL SET /p elidenewline=Compiling %TARGET%... 
%GXX% -o %TARGET% %SOURCES% -std=c++11 -lm -g -Wno-unused-result -Ilibinteractive/ColaInteractivo
IF !ERRORLEVEL! NEQ 0 EXIT /B !ERRORLEVEL!
ECHO DONE
)
